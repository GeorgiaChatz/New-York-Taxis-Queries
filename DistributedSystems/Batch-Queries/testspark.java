...
public class testspark {
    ...
    //in this query we count the taxi that began from each quadrant, for each day of a month
    ...
    //in this query we return the number of taxi calls that happened, for every hour
    public static void fourthQueryBatch() {
        System.out.println("--------Fourth Query---------");
        //System.out.println(df.count());
        df.groupBy(df.col("hour")).count().show();
        //df.groupBy(functions.hour(df.col("pickup_datetime"))).count().show(24);
        //System.out.print( df.agg(functions.sum("count")));
    }

    //in this query the user gives longitude, latitude and time
    //and we return how many taxis began that time from the same quadrant
    public static void fifthQuery() {
        ...
        //take the info from the user with comma
    ...
    public static void sixthQueryBatch() {
        ...
        // df.show();
        //df.agg(functions.max(df.col("count"))).show();
    }

    //in this method we find the hour that each vector (1 or 2) has the most taxi calls
    ...
    //in this query we print the taxi calls per hour, for weekends
    ...
    public static void VendorCallsPerHourStream() {
        // df.groupBy(window(df.col("pickup_datetime"),"1 day","1 hour")).count().orderBy(df.col("pickup_datetime")).show(50);
        //df.filter(df.col("vendor_id").equalTo(2)).groupBy(window(df.col("pickup_datetime"),"1 day","1 hour")).count().show();
