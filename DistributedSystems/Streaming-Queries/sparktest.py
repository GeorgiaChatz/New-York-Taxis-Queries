...
df = df.groupBy(df["Hour"]).count()#edw thelei completeb
#-------------------THIRD STREAMING QUESTION------------------------------------
#df = df.groupBy(df["DayOfYear"], df["vendor_id"], df["Hour"]).count()#.agg(max("count"))#requires double aggregation
#df = df.agg(max("count"))
#df = df.groupBy(df["hour"], df["vendor_id"]).count()#edw thelei complete
